...
void QExcel::Quit(QAxObject *book) const
{
    book->dynamicCall("Close()");
}

QAxObject* QExcel::Open(const QString &path)
{
    QDir dir(path);
   QFile file(path);
   ...
   if (file.exists())
   {
       return _WorkBooks->querySubObject("Open(const QString &)", dir.absolutePath());
   }
   else {
...
{
    return ranges->querySubObject("Replace(const QVariant&, const QVariant&, const QVariant&, const QVariant&, const QVariant&, const XlSearchDirection&, const QVariant&)",
...
//    QAxObject* shapes = sheet->querySubObject("Shapes");
    ...
    {
        _lastError="file.open无法载入图片："+picPath;
        qDebug()<<_lastError;
        return;
    }
    ...
    QAxObject* picture = pictures->querySubObject("Insert(const QByteArray&)", buffer.data());
    if(!picture){
        _lastError="QAxObject无法载入图片："+picPath;
        qDebug()<<_lastError;
        return;
    }
...
//    QObject::connect(shapes, SIGNAL(exception(int,QString,QString,QString)), this, SLOT(onException(int,QString,QString,QString)));
////    connect(shapes,&QAxObject::exception,this,&QExcel::onException);
